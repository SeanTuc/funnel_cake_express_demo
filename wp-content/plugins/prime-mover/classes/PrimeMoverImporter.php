...
class PrimeMoverImporter implements PrimeMoverImport
{
    ...
    public function maybeProcessThirdPartyData($ret = [], $blogid_to_import = 0, $files_array = [], $start_time = 0)
    {
        ...
        /**
         * Notes: refer to developer notes for using this filter with heading:
         * How to add more third party processing functions?
         */
    ...
    {        
        ...
        do_action('prime_mover_log_processed_events', 'MySQL import command: ', $blogid_to_import, 'import', $current_func, $this);
        do_action('prime_mover_log_processed_events', $import_command, $blogid_to_import, 'import', $current_func, $this);
        ...
        do_action('prime_mover_log_processed_events', "Database restore call with args:" , $blogid_to_import, 'import', $current_func, $this);
        do_action('prime_mover_log_processed_events', $ret , $blogid_to_import, 'import', $current_func, $this, true);
    ...
    protected function restoredB($restore_commands = [], $ret = [], $blogid_to_import = 0, $start_time = 0)
    {
        ...
        fclose($handle);        
        if ($max_allowed_packet_error) {
            $return['error'] = sprintf(esc_html__('Error: Your MySQL server max_allowed_packet size is insufficient. Please consider increasing this above %d bytes.'), $max_allowed_packet);
        } else {
     ...
     */
    protected function killBlockingProcesses()
    {       
        global $wpdb;
    ...
    {        
        ...
        do_action('prime_mover_log_processed_events',"Footprint parameters: ", $blogid_to_import, 'import', __FUNCTION__, $this);
        do_action('prime_mover_log_processed_events',$ret, $blogid_to_import, 'import', __FUNCTION__, $this);
    }
    ...
    public function markTargetSiteUploadsInformation($ret = [], $blogid_to_import = 0, $files_array = [], $start_time = 0)
    {
        ...
        $this->getSystemFunctions()->switchToBlog($blogid_to_import);
        $ret['canonical_uploads_information'] = wp_upload_dir(null, true, true);
    ...
    public function saveImportProgressData($ret = [], $blogid_to_import = 0, $next_method = '', $current_method = '')
    {
        ...
        update_user_meta($user_id, $meta_key, $ret);
