...
{    
    ...
    public function primeMoverGetBlogsDirPath($blog_id = 0, $dirname = true, $ret = [])
    {
        ...
        if ( ! empty($ret['canonical_uploads_information']) ) {
            ...
            $uploads_information = wp_upload_dir();
        }
    ...
    public function primeMoverGetBlogsDirUrl($blog_id = 0, $ret = [])
    {
        ...
        if ( ! empty($ret['canonical_uploads_information']) ) {
            ...
            $uploads_information = wp_upload_dir(null, true, true);
        }        
     ...
     */
    public function cleanDbTablesForExporting($inputs = [], $blog_id = 0, \wpdb $wpdb, $mode = 'export')
    {
        if (!$this->getSystemAuthorization()->isUserAuthorized()) {
            return;
        }
     ...
     */
    public function updateBlogOption($blog_id = 0, $option = '', $value)
    {
        if ( ! $this->getSystemAuthorization()->isUserAuthorized()) {
            return;
        }
     ...
     */
    public function updateSiteOption($key = '', $value, $force_delete_cache = false) 
    {
        $ret = false;
        if ( ! $this->getSystemAuthorization()->isUserAuthorized()) {
            return $ret;
    ...
    {
        if (function_exists('loadOnlyPrimeMoverPlugin')) {            
    ...
    {
        remove_filter('option_active_plugins', 'loadOnlyPrimeMoverPlugin');
        remove_filter('site_option_active_sitewide_plugins', 'loadOnlyPrimeMoverPlugin');
        remove_filter('stylesheet_directory', 'disableThemeOnPrimeMoverProcesses', 10000);
        remove_filter('template_directory', 'disableThemeOnPrimeMoverProcesses', 10000);
    }
    ...
    {
        $panel_identifier = 'prime-mover_page_migration-panel-basic-settings-network';
        $advance_panel_identifier = 'prime-mover_page_migration-panel-advance-settings-network';
        if ( ! is_multisite() ) {
            $panel_identifier = 'prime-mover_page_migration-panel-basic-settings';
            $advance_panel_identifier = 'prime-mover_page_migration-panel-advance-settings';
        }
    ...
    public function maybeLoadAssetsOnDashboard(WP_Screen $current_screen)
    {
        ...
        $account_page_id = 'prime-mover_page_migration-panel-settings-account';
        if (is_multisite()) {
            $account_page_id = 'prime-mover_page_migration-panel-settings-account-network';
        }
     ...
     * @codeCoverageIgnore
     */
    protected function maybeUseStoreMode($store_mode = false, ZipArchive $ziparchive, $local_name = '')
    {
    ...
    public function isFileResideInExportDir($filepath = '')
    {
        ...
        $export_directory = wp_normalize_path($this->getSystemInitialization()->getMultisiteExportFolderPath());
        if (false !== strpos($filepath, $export_directory)) {
            $ret = true;
        }
        return $ret;
    }
    ...
    public function isPrimeMoverPage()
    {
        ...
        if ( $this->maybeLoadAssetsOnDashboard($current_screen)) {
            return true;
        }
