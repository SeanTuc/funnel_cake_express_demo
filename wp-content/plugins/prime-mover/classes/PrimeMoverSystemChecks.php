...
{       
     ...
     * @return array
     */
    protected function closeArchiveAndReturn($file = '', $blog_id = 0, $processed = [], ZipArchive $zip, $start_time = 0, $total_sizes = 0, $count = 0, $large_file = false, $filesize = 0, $batch_bytes = 0, $position = 0, $ret = [])
    {
        if ( ! $file || ! $blog_id) {
            return;
        }
     ...
     * @return array
     */
    protected function addEmptyDirectoryToArchive($source = '', $ret = [], $exporting_mode = '', $file = '', $returned_array = [], $count = 0, $key = '', $encrypt = false, ZipArchive $zip, $blog_id = 0, $shell_mode = false)
    {
        $add_result = false;
     ...
     * @param number $blog_id
     */
    protected function maybeOptimizePerformanceForLargeFileInArchive($filesize = 0, ZipArchive $zip, $entity = '', $blog_id = 0)
    {
        if ( ! $entity ) {
            return;
        }
     ...
     * @return boolean
     */
    protected function addFileOnWindows($filename = '', $localname = '', $filesize = 0, ZipArchive $zip)
    {
        if (!$this->isWindows()) {
            return false;  
        }
