...
{         
    ...
    public function dBCustomerUserIdsHelper($ret = [], $table = '', $blogid_to_import = 0, $leftoff_identifier = '', $primary_index = '', $column_strings = '',
        $update_variable = '', $progress_identifier = '', $start_time = 0, $last_processor = false, $handle_unique_constraint = '')
    {
        if (!$this->getSystemAuthorization()->isUserAuthorized()) {
            return $ret;
        }
        ...
        $query = $this->seekCustomersToUpdateQuery($ret, $leftoff_identifier, $table, $primary_index, $column_strings);
        $customers_updated = 0;
        ...
        while ($results = $wpdb->get_results($query, ARRAY_A)) {
            if (empty($results)) {
                ...
                $ret = $this->updateCustomerIds($results, $user_equivalence, $ret, $update_variable, $column_strings, $table, $leftoff_identifier);
            }
        ...
        $ret = $this->cleanUpRetArrayAfterCustomerIdProcessing($ret, $leftoff_identifier, $update_variable, $last_processor, $handle_unique_constraint, $table, $wpdb);
        $this->getSystemFunctions()->restoreCurrentBlog();
     ...
     * @param string $index
     */
    protected function dropIndexesConstraint($table = '', wpdb $wpdb, $index = '')
    {
     ...
     * @return string
     */
    protected function seekCustomersToUpdateQuery($ret = [], $leftoff_identifier = '', $table = '', $primary_index = '', $column_strings = '')
    {
        global $wpdb;
     ...
     * @return array
     */
    protected function updateCustomerIds($results = [], SplFixedArray $user_equivalence, $ret = [], $update_variable = '', $column_strings = '',
        $table = '', $leftoff_identifier = '')
    {
        if (empty($results)) {
            return;
        }
        ...
        foreach ($results as $result) {
            ...
            $this->maybeEnableUserImportExportTestMode(3, false);
     ...
     */
    private function updateCustomerUserIdBySQL($primary_index_id = 0, $migrated_user_id = 0, $table = '', $primary_index = '', $user_id_column = '')
    {
        global $wpdb;
        $prep = $wpdb->prepare("
    ...
    protected function cleanUpRetArrayAfterCustomerIdProcessing($ret = [], $leftoff_identifier = '', $update_variable = '',
        $last_processor = false, $handle_unique_constraint = '', $table = '', wpdb $wpdb)
    {
        if (isset($ret[$leftoff_identifier])) {
     ...
     * @param string $index
     */
    protected function enForceIndexesConstraint($table = '', wpdb $wpdb, $index = '')
    {
    ...
    public function maybeRequisitesNotMeetForAdjustment($ret = [], $blogid_to_import = 0, $target_plugin = '')
    {
        ...
        if (!$mismatch_count ) {
            do_action('prime_mover_log_processed_events', "Post mismatch count is zero, skipping WC customer lookup update.", $blogid_to_import, 'import', __FUNCTION__, $this);
            return $ret;
        }
